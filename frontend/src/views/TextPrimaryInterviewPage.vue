<template>
  <div class="text-primary-interview-page">
    ...
    <main class="interview-main">
        ...
        <section class="question-control-section">
          ...
          <div class="candidate-info-card">
            <div class="card-header">
              ...
              <div class="header-right">
                <el-tag type="success" size="small">AI面试中</el-tag>
              </div>
            </div>
            <div class="card-content">
              ...
              <div class="info-item">
                ...
                <span class="value score-highlight">{{ Math.round(realTimeAnalysis.overallScore || 0) }}分</span>
              </div>
            </div>
          </div>
      ...
      <!-- 分析面板区域 - 放在面试对话和候选人信息下方 -->
      <div class="analysis-panels-external">
        <!-- 实时分析状态面板 -->
        <div class="analysis-panel-external status-panel-external">
          ...
          <div class="panel-content">
            <div class="processing-stats">
              ...
              <div class="stat-item">
                <span class="stat-label">沟通技巧</span>
                <span class="stat-value">{{ realTimeAnalysis.communicationSkills || 0 }}</span>
              </div>
              <div class="stat-item">
                <span class="stat-label">表达能力</span>
                <span class="stat-value">{{ realTimeAnalysis.expressionAbility || 0 }}</span>
              </div>
              <div class="stat-item">
        ...
        <!-- 文本分析结果面板 -->
        <div class="analysis-panel-external results-panel-external">
          <div class="panel-header">
            <div class="header-left">
              <el-icon><Document /></el-icon>
              <h4>文本分析结果</h4>
      ...
      <div class="ai-hints-floating-panel">
        ...
        <div class="hints-content">
          <div class="hint-text-container">
            <p class="hint-text">{{ currentAiHint.hint || currentAiHint }}</p>

            <!-- 增强提示信息 -->
            <div v-if="currentAiHint.guidance" class="hint-guidance">
              <div class="guidance-label">
                <el-icon><Guide /></el-icon>
                <span>引导建议</span>
              </div>
              <p class="guidance-text">{{ currentAiHint.guidance }}</p>
            </div>

            <!-- 示例提示 -->
            <div v-if="currentAiHint.examples && currentAiHint.examples.length > 0" class="hint-examples">
              <div class="examples-label">
                <el-icon><List /></el-icon>
                <span>可以从这些方面展开</span>
              </div>
              <ul class="examples-list">
                <li v-for="example in currentAiHint.examples" :key="example" class="example-item">
                  {{ example }}
                </li>
              </ul>
            </div>
            ...
            <!-- 提示来源和类型 -->
            <div v-if="currentAiHint.source" class="hint-meta">
              <el-tag
                :type="getHintSourceType(currentAiHint.source)"
                size="small"
                class="source-tag"
              >
                {{ getHintSourceLabel(currentAiHint.source) }}
              </el-tag>
              <el-tag
                v-if="currentAiHint.urgency"
                :type="getUrgencyType(currentAiHint.urgency)"
                size="small"
                class="urgency-tag"
              >
                {{ getUrgencyLabel(currentAiHint.urgency) }}
              </el-tag>
            </div>
          </div>
...
import {
  ...
  VideoPlay, SwitchButton, Guide, List
...
const generateHumanizedThinkingProcess = (analysis, userAnswer) => {
  ...
  if (interviewState.value.consecutiveUnknownAnswers >= 2) {
    ...
    // 修复：确保对话流程评分正确显示
    const actualCurrentScore = currentScore || 0
    const actualTotalTurns = flow.totalTurns || 0
    const actualMeaningfulTurns = flow.meaningfulTurns || 0
    const actualConceptsCovered = flow.conceptsCovered ? flow.conceptsCovered.size : 0
    const actualTopicDepth = flow.topicDepth || 0

    thinking += `对话流程分析：单次回答${score}分，对话流程累积${actualCurrentScore}分，总轮次${actualTotalTurns}，有意义轮次${actualMeaningfulTurns}，概念覆盖${actualConceptsCovered}个，深度层级${actualTopicDepth}。`

    if (actualCurrentScore >= 85) {
      thinking += `对话流程评分${actualCurrentScore}分，这是一个卓越的对话表现！候选人不仅在技术内容上表现出色，整个对话过程展现了深度思考、概念整合和实践经验。基于对话流程的完整性和质量，这个问题已经得到了充分的探讨。`
      interviewState.value.shouldTransitionToNext = true
    } else if (actualCurrentScore >= 75) {
      thinking += `对话流程评分${actualCurrentScore}分，整体对话质量很好！候选人在技术理解、概念运用和表达逻辑方面都表现不错，对话呈现良好的递进性。这个问题基本已经得到了充分的回答。`
      interviewState.value.shouldTransitionToNext = true
    } else if (actualCurrentScore >= 60) {
      thinking += `对话流程评分${actualCurrentScore}分，对话质量良好，显示了一定的技术基础。我可以通过追问来进一步了解候选人的深度理解，或者根据对话完整性考虑下一步。`
      interviewState.value.shouldTransitionToNext = false
    } else if (actualCurrentScore >= 40) {
      thinking += `对话流程评分${actualCurrentScore}分，对话显示出基础理解，但在深度、实例和概念整合方面还有提升空间。我需要通过引导来帮助候选人更好地展示能力。`
      interviewState.value.shouldTransitionToNext = false
    } else {
      thinking += `对话流程评分${actualCurrentScore}分，对话质量需要改善。我应该提供更多具体的指导和鼓励，帮助候选人建立信心并改善表达方式。`
      interviewState.value.shouldTransitionToNext = false
    }
...
const getAiHint = async () => {
  ...
  try {
    ...
    // 支持增强提示对象和简单字符串
    currentAiHint.value = hint
    showAiHints.value = true
    aiAssistanceCount.value--

    // 根据提示类型显示不同的成功消息
    const successMessage = hint.source === 'iflytek_enhanced'
      ? 'iFlytek智能提示已生成'
      : 'AI智能提示已生成'
...
// 获取提示来源类型
const getHintSourceType = (source) => {
  const typeMap = {
    'iflytek_enhanced': 'success',
    'local_enhanced': 'primary',
...
// 获取提示来源标签
const getHintSourceLabel = (source) => {
  const labelMap = {
    'iflytek_enhanced': 'iFlytek智能分析',
    'local_enhanced': '本地智能分析',
    'local_fallback': '基础提示'
  }
  return labelMap[source] || '智能提示'
}
...
const calculateConversationScore = (userAnswer, aiResponse, singleScore) => {
  ...
  // 7. 修复：确保所有指标都有有效值
  if (!flow.conceptsCovered) {
    flow.conceptsCovered = new Set()
  }
  if (typeof flow.totalTurns !== 'number') {
    flow.totalTurns = 1
  }
  if (typeof flow.meaningfulTurns !== 'number') {
    flow.meaningfulTurns = scoreIncrement > rules.baseIncrement * 0.8 ? 1 : 0
  }
  if (typeof flow.topicDepth !== 'number') {
    flow.topicDepth = 0
  }
...
const calculateOverallPerformance = () => {
  const questionHistory = conversationScoring.value.questionHistory || []
  const currentScore = conversationScoring.value.currentQuestionScore || 0
  const currentFlow = conversationScoring.value.conversationFlow

  // 包含当前问题的所有分数
  const allScores = [...questionHistory.map(q => q.score), currentScore]
  const allTurns = [...questionHistory.map(q => q.totalTurns), currentFlow.totalTurns]
  const allMeaningfulTurns = [...questionHistory.map(q => q.meaningfulTurns), currentFlow.meaningfulTurns]
  const allConceptsCovered = [...questionHistory.map(q => q.conceptsCovered), currentFlow.conceptsCovered.size]

  const averageScore = allScores.length > 0 ? Math.round(allScores.reduce((sum, score) => sum + score, 0) / allScores.length) : 0
  const totalTurns = allTurns.reduce((sum, turns) => sum + turns, 0)
  const totalMeaningfulTurns = allMeaningfulTurns.reduce((sum, turns) => sum + turns, 0)
  const totalConceptsCovered = allConceptsCovered.reduce((sum, concepts) => sum + concepts, 0)

  return {
    averageScore,
    totalTurns,
    totalMeaningfulTurns,
    totalConceptsCovered,
...
const resetConversationScoring = () => {
  ...
  const currentPerformance = {
    questionNumber: currentQuestion.value,
    score: conversationScoring.value.currentQuestionScore,
    totalTurns: conversationScoring.value.conversationFlow.totalTurns,
    meaningfulTurns: conversationScoring.value.conversationFlow.meaningfulTurns,
    conceptsCovered: conversationScoring.value.conversationFlow.conceptsCovered.size,
    topicDepth: conversationScoring.value.conversationFlow.topicDepth,
    timestamp: new Date().toISOString()
  }

  // 添加到历史记录
  if (!conversationScoring.value.questionHistory) {
    conversationScoring.value.questionHistory = []
  }
  conversationScoring.value.questionHistory.push(currentPerformance)

  // 计算历史平均表现
  const avgPerformance = conversationScoring.value.questionHistory.length > 0
    ? conversationScoring.value.questionHistory.reduce((sum, q) => sum + q.score, 0) / conversationScoring.value.questionHistory.length
    : 0

  console.log('📊 保存当前问题表现:', currentPerformance)
  console.log('📈 历史平均表现:', avgPerformance)
...
.interview-header {
  ...
  padding: 16px 32px; /* 减少上下内边距，让header更紧凑 */
...
.interview-main {
  flex: 1;
  padding: 24px;
  min-height: calc(100vh - 70px); /* 改为最小高度，允许内容扩展 */
...
.interview-layout {
  ...
  grid-template-columns: 1fr 420px !important; /* 增加右侧边栏宽度到420px */
  gap: 24px;
  max-width: 1500px; /* 增加最大宽度以适应更宽的右侧边栏 */
  margin: 0 auto;
  height: auto; /* 改为自动高度，为下方分析面板留出空间 */
  min-height: calc(80vh - 20px); /* 显著增加最小高度，确保容器能容纳增加的内容高度 */
...
.conversation-section {
  ...
  height: auto; /* 自动高度 */
  min-height: calc(85vh - 40px); /* 显著增加最小高度，确保与右侧三个组件总高度精确对齐 */
  max-height: calc(95vh - 20px); /* 增加最大高度，提供充足的对话空间 */
  max-width: 1400px;
  width: 100%;
  margin: 0 auto;
  overflow-y: auto; /* 允许垂直滚动 */
...
.messages-container {
  ...
  flex-direction: column;
  gap: 20px; /* 增加消息间距 */
  min-height: 600px; /* 大幅增加最小高度，充分利用纵向空间 */
  max-height: none; /* 移除最大高度限制，让它自由伸展 */
  /* 自定义滚动条样式 */
...
.current-question-panel {
  ...
  overflow-y: visible; /* 移除滚动，让内容完全显示 */
...
.answer-input-panel {
  border: 1px solid #d9d9d9;
  border-radius: 8px;
  overflow: visible; /* 改为可见，确保按钮区域不被截断 */
  flex-shrink: 0; /* 防止被压缩 */
...
.analysis-section {
  ...
  gap: 16px;
  height: 100%; /* 占满父容器高度 */
  min-height: calc(100vh - 150px); /* 调整最小高度，与左侧对话框保持一致 */
...
@media (max-width: 1024px) {
  .conversation-section {
    padding: 24px;
    min-height: calc(80vh - 60px); /* 中等屏幕下增加最小高度以保持对齐 */
    max-height: calc(90vh - 30px); /* 中等屏幕下增加最大高度 */
    max-width: 100%;
  }
...
@media (max-width: 480px) {
  .conversation-section {
    padding: 16px;
    min-height: calc(75vh - 80px); /* 移动端调整为最小高度而非固定高度 */
    max-height: calc(85vh - 60px); /* 移动端增加最大高度 */
    border-radius: 8px;
  }
...
@media (max-width: 480px) {
  .conversation-section {
    padding: 12px;
    min-height: calc(70vh - 60px); /* 小屏幕移动端调整最小高度 */
    max-height: calc(80vh - 40px); /* 小屏幕移动端增加最大高度 */
  }
...
@media (max-width: 1024px) and (min-width: 481px) {
  .interview-layout {
    grid-template-columns: 1fr 350px !important; /* 中等屏幕下保持较宽的右侧边栏 */
    max-width: 1200px; /* 调整最大宽度 */
    min-height: calc(75vh - 30px); /* 中等屏幕下增加最小高度以保持对齐 */
  }

  /* 中等屏幕下的分析面板调整 */
  .analysis-panels-container {
    max-width: 1200px; /* 与interview-layout保持一致 */
    gap: 20px; /* 稍微减少间距 */
  }

  .status-panel-external,
  .results-panel-external {
    flex: 0 0 500px; /* 中等屏幕下统一宽度 */
    min-width: 500px;
    max-width: 500px;
    height: 260px; /* 中等屏幕下增加高度以容纳6个项目 */
    border-radius: 18px; /* 中等屏幕下统一圆角 */
    padding: 16px; /* 中等屏幕下调整内边距 */
    overflow: visible; /* 改为可见，确保所有内容显示 */
  }
  ...
  .analysis-panel .panel-header h4 {
    font-size: 17px; /* 中等屏幕下稍微减小字体 */
  }

  .stat-value {
    font-size: 16px; /* 中等屏幕下调整数值字体大小 */
  }
}
...
@media (max-width: 1024px) and (min-width: 481px) {
  .analysis-panels-external {
    gap: 16px;
    margin: 4px auto 20px auto; /* 将上边距从20px减少到4px，向上移动16px */
    flex-direction: column; /* 中等屏幕下垂直排列 */
    align-items: center;
  }
...
@media (max-width: 480px) {
  ...
  .analysis-panels-container {
    ...
    max-width: 100%; /* 移动端全宽 */
    gap: 16px; /* 移动端减少间距 */
    padding: 16px; /* 移动端恢复内边距 */
  }

  .analysis-panels-external {
    flex-direction: column;
    gap: 16px;
    margin: 0px auto 16px auto; /* 将上边距从16px减少到0px，向上移动16px */
    align-items: center;
  }
  ...
  .results-panel-external {
    ...
    min-width: 100%;
    width: 100%;
    height: 220px; /* 移动端增加高度以容纳6个项目 */
    flex: none; /* 移动端取消flex设置 */
    border-radius: 16px; /* 移动端统一圆角 */
    padding: 14px; /* 移动端统一内边距 */
    overflow: visible; /* 改为可见，确保所有内容显示 */
  }
  ...
  .status-panel-external .panel-header,
  .results-panel-external .panel-header {
    height: 35px;
    margin-bottom: 6px;
  }

  .status-panel-external .panel-header h4,
  .results-panel-external .panel-header h4 {
    font-size: 12px;
  }
  ...
  .stat-value {
    font-size: 15px; /* 移动端数值字体 */
  }

  .status-panel .status-indicator {
    font-size: 13px; /* 移动端状态指示器字体 */
  }

  .conversation-section {
    min-height: calc(65vh - 100px); /* 移动端调整最小高度以适应小屏幕 */
    max-height: calc(75vh - 80px); /* 移动端调整最大高度 */
...
.question-control-section {
  ...
  display: flex;
  flex-direction: column;
  gap: 18px; /* 稍微减少间距以优化底部对齐 */
  padding: 0;
  min-height: calc(85vh - 40px); /* 与左侧对话区域高度精确对齐 */
  justify-content: flex-start; /* 内容从顶部开始排列 */
}

/* 确保候选人信息卡片在右侧面板中正确显示 */
.question-control-section .candidate-info-card {
  order: -1; /* 确保候选人信息显示在最上方 */
  margin-bottom: 18px; /* 与gap保持一致，优化对齐 */
}

.current-question-panel {
  background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
  border-radius: 16px;
  padding: 24px; /* 增加内边距以适应更宽的面板 */
  color: white;
...
.question-buttons {
  ...
  gap: 10px; /* 增加按钮间距 */
  max-width: 360px; /* 增加最大宽度以适应更宽的右侧边栏 */
  width: 100%; /* 确保占满容器宽度 */
  align-items: center;
...
.candidate-info-card .card-header {
  display: flex;
  align-items: center;
  justify-content: space-between;
  padding: 18px 20px 14px 20px; /* 增加内边距以适应更宽的卡片 */
  border-bottom: 1px solid rgba(240, 240, 240, 0.8);
}

.candidate-info-card .card-content {
  padding: 14px 20px 18px 20px; /* 增加内边距 */
}

.candidate-info-card .header-left {
  display: flex;
  align-items: center;
  gap: 8px;
}

.candidate-info-card .header-left h3 {
  margin: 0;
  font-size: 15px;
  font-weight: 600;
  color: #262626;
...
.candidate-info-card .info-item {
  display: flex;
  justify-content: space-between;
  align-items: center;
  margin-bottom: 12px; /* 增加间距 */
  padding: 4px 0; /* 增加内边距 */
  min-height: 24px; /* 确保一致的高度 */
}

.candidate-info-card .info-item:last-child {
  margin-bottom: 0;
}

.candidate-info-card .label {
  font-size: 14px;
  color: #8c8c8c;
  font-weight: 500;
}

.candidate-info-card .value {
  font-size: 14px;
  color: #262626;
  font-weight: 600;
}

.candidate-info-card .score-highlight {
  color: #52c41a;
  font-weight: 700;
}
...
.analysis-panels-container {
  display: flex;
  flex-direction: row;
  gap: 24px; /* 与上方interview-layout的gap保持一致 */
  max-width: 1500px; /* 与interview-layout的max-width保持一致 */
  margin: 60px auto 20px auto; /* 增加顶部间距，向下移动 */
  align-items: stretch;
  width: 100%;
  height: auto;
  min-height: 120px;
...
/* 向下移动的分析面板容器 */
.analysis-panels-container.moved-down {
  margin-top: 80px; /* 进一步向下移动 */
}

/* 白色方框内的分析面板容器 - 横向排列 */
.analysis-panels-in-conversation {
  display: flex;
...
.analysis-panels-external {
  ...
  gap: 20px;
  max-width: 1400px;
  margin: 8px auto 20px auto; /* 将上边距从24px减少到8px，向上移动16px */
  align-items: stretch;
  width: 100%;
...
/* 独立分析面板样式 */
.analysis-panel-external {
  background: rgba(255, 255, 255, 0.95);
  backdrop-filter: blur(20px);
  border-radius: 20px; /* 统一圆角设计 */
  padding: 24px;
  box-shadow:
    0 8px 32px rgba(0, 0, 0, 0.12),
    0 2px 8px rgba(0, 0, 0, 0.08),
...
/* 左侧实时分析状态面板 - 统一尺寸和圆角 */
.status-panel-external {
  flex: 0 0 680px; /* 固定宽度，与右侧面板相同 */
  min-width: 680px;
  max-width: 680px;
  height: 280px; /* 增加高度以容纳6个项目，从220px增加到280px */
  border-left: 4px solid #52c41a;
  padding: 18px; /* 调整内边距 */
  border-radius: 20px; /* 统一圆角设计 */
  background: rgba(255, 255, 255, 0.98);
...
/* 实时分析状态面板状态指示器样式 */
.status-panel-external .status-indicator {
  display: flex;
  align-items: center;
  gap: 6px;
  font-size: 12px;
...
.status-panel-external .status-dot {
  width: 8px;
  height: 8px;
  border-radius: 50%;
  background: #52c41a;
...
/* 文本分析结果面板顶部信息区域 */
.results-panel-external .panel-header .header-right {
  display: flex;
  gap: 8px;
  font-size: 11px;
}

.results-panel-external .panel-header .header-right .el-tag {
  font-size: 10px;
  padding: 2px 6px;
  height: auto;
  line-height: 1.2;
...
/* 文本分析结果面板内容样式优化 */
.results-panel-external .analysis-content {
  font-size: 13px;
  line-height: 1.4;
  color: #262626;
  flex: 1;
...
.results-panel-external .analysis-item {
  margin-bottom: 8px;
  padding: 6px 0;
  display: flex;
  justify-content: space-between;
...
.results-panel-external .analysis-label {
  font-size: 12px;
  color: #8c8c8c;
  font-weight: 500;
  flex: 1;
...
.results-panel-external .analysis-value {
  font-size: 14px;
  font-weight: 600;
  color: #1890ff;
  flex-shrink: 0;
...
.results-panel-external .score-breakdown {
  ...
  grid-template-rows: 1fr 1fr 1fr; /* 增加为3行以容纳6个项目 */
  gap: 8px; /* 稍微减少间距以适应更多内容 */
  margin-top: -4px; /* 向上移动16px，从12px调整为-4px */
  flex: 1;
}
...
.results-panel-external .score-item {
  ...
  flex-direction: column;
  align-items: center;
  justify-content: flex-start; /* 改为顶部对齐，使内容向上 */
  padding: 3px 5px 6px 5px; /* 进一步减少padding以适应3行布局 */
  background: #f8f9fa;
  border-radius: 8px;
  text-align: center;
  min-height: 42px; /* 减少最小高度以适应3行布局 */
}

.results-panel-external .score-name {
  font-size: 11px;
  color: #8c8c8c;
  margin-bottom: 2px; /* 减少底部间距，使数字更贴近标签 */
  white-space: nowrap;
  overflow: hidden;
  text-overflow: ellipsis;
  width: 100%;
